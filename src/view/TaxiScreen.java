...
public class TaxiScreen extends Form implements CommandListener {
	...
	public TaxiScreen(Business b) {
		...
		ImageItem imageItem;
		try {
			imageItem = new ImageItem(null, Image.createImage("/taximain.png"),
	...
	public void commandAction(Command arg0, Displayable arg1) {
		if (arg0 == this.cmdExit) {
			TaxiMIDlet.getInstance().exitNow();
			return;
		} else if (arg0 == this.cmdHelp) {
			TaxiMIDlet.getInstance().setCurrentScreen(new HelpScreen());

		} else if (arg0 == this.cmdAbout) {
			TaxiMIDlet.getInstance().setCurrentScreen(new AboutScreen());
			...
			TaxiMIDlet.getInstance().setCurrentScreen(new PretsScreen("Prêts"));
		} else if (arg0 == this.cmdPertes) {
			TaxiMIDlet.getInstance()
					.setCurrentScreen(new PertesScreen("Pertes"));
		} else if (arg0 == this.cmdDetails) {
			TaxiMIDlet.getInstance().setCurrentScreen(
					new DetailsScreen("Détails"));
		} else if (arg0 == this.cmdSMS) {
			TaxiMIDlet.getInstance().setCurrentScreen(new SMSScreen("SMS"));
		} else if (arg0 == this.cmdRAZ) {
			TaxiMIDlet.getInstance().setCurrentScreen(alertConf);
		} else if (arg0 == cmdOui) {
